...
Essa será uma aplicação para gerenciar tarefas (*todos*). Será permitida a criação de um usuário com `name` e `username`, bem como fazer o CRUD de *todos*:
...
A rota deve receber `title` e `deadline` dentro do corpo da requisição, e uma propriedade `username` contendo o username do usuário dentro do header da requisição. Ao criar um novo *todo*, ele deve ser armazenado dentro da lista `todos` do usuário que está criando essa tarefa. Certifique-se que o ID seja um UUID. Cada tarefa deverá estar no seguinte formato:
...
Para que esse teste passe, você deve permitir que um usuário seja criado e retorne um JSON com o usuário criado. O formato esperado é o seguinte:
